...
function scoreBreakdown(analysis: SymbolState['analysis'], language: Language) {
  ...
  const labels = language === 'ar'
    ? [['technical', 'فني', b.technicalScore], ['news', 'أخبار', b.newsScore], ['macro', 'ماكرو', b.macroScore], ['historical', 'تاريخي', b.historicalScore], ['risk', 'مخاطرة', b.riskScore]]
    : [['technical', 'Technical', b.technicalScore], ['news', 'News', b.newsScore], ['macro', 'Macro', b.macroScore], ['historical', 'Historical', b.historicalScore], ['risk', 'Risk', b.riskScore]];
  return `<div class="score-breakdown">${labels.map(([kind, label, value]) => `<span class="score-chip ${kind}"><small>${escapeHtml(String(label))}</small><b>${Number(value)}/100</b></span>`).join('')}</div>`;
}
